...
def generate_all_charts(
    ...
    Args:
        ...
        benchmarks: Dict of benchmark name â†’ BenchmarkResult.
...
def _regime_price_chart(result, prices: pd.DataFrame, output_dir: str) -> str:
    ...
    if result.config_metadata:
        equity_label = result.config_metadata.get("configured_tickers", {}).get("equity", equity_label)
    if "equity" in prices.columns:
        eq = prices["equity"].dropna()
        ax.plot(eq.index, eq.values, label=equity_label, color="#1f77b4", linewidth=1.5)
    ax.set_ylabel("Price", fontsize=11)
    ax.set_title(f"{equity_label} with Regime Background", fontsize=13)
...
def _transition_matrix_chart(result, output_dir: str) -> str:
    ...
    if trans_mat is None or trans_mat.empty:
        raise ValueError(
            "Transition matrix is absent â€” the HMM must have been successfully "
...
def _portfolio_weights_chart(result, output_dir: str) -> str:
    ...
    # Only plot rows where allocation is active (sum of weights â‰ˆ 1)
    # Pre-allocation rows are all-zero â€” skip them.
...
def _regime_probabilities_chart(result, output_dir: str) -> str:
    ...
    # Drop rows before the first successful rebalance â€” those rows have no
